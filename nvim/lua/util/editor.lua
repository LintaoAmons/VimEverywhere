...
---@param bufnr? integer
local function get_buf_relative_dir_path(bufnr)
  bufnr = bufnr or vim.api.nvim_get_current_buf()
  local full_path = vim.fn.fnamemodify(vim.api.nvim_buf_get_name(bufnr), ":.")
  local project_path = find_project_path() or ""
  return string.sub(full_path, string.len(project_path) + 2, string.len(full_path))
end
